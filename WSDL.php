...
class WSDL extends Base
{
    ...
    /**
     * [$portTypes description]
     * @var array
     */
    public $portTypes = array();
    
    /**
     * [$currentPortType description]
     * @var [type]
     */
     ...
     * @param array $curlOptions user-specified cURL options
     * @param boolean $useCurl try to use cURL
     * @access public 
     */
                        ...
                        for ($ii = 0; $ii < count($list2); $ii++)
                        {
                            if (! $list2[$ii]['loaded'])
                            {
                                ...
                                if ($url != '')
                                {
                                    ...
                                    {
                                        $url = $this->wsdlParts['scheme'] . '://' . $this->wsdlParts['host'] . (isset($this->wsdlParts['port']) ? ':' .$this->wsdlParts['port'] : '') .
                                                substr($this->wsdlParts['path'],0,strrpos($this->wsdlParts['path'],'/') + 1) .$urlparts['path'];
                                    }
    ...
    protected function _parseWSDL($wsdl = '')
    {
        ...
        xml_set__characterData_handler($this->parser, '_characterData');
        // Parse the XML file.
        ...
        if (isset($opData[$direction]['parts']) && sizeof($opData[$direction]['parts']) > 0)
        {
            ...
            if (is_array($parameters))
            {
                ...
                foreach ($parts as $name => $type)
                {
                    ...
                    // if _serializeType returns false, then catch global error and fault
        ...
        if (strpos($type, ':'))
        {
            ...
            if ($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/')
            {
                ...
                if ($uqType == 'Array')
                {
                    ...
                    return $this->serializeVal($value, $name, false, false, false, false, $use);
                }
        ...
        else if ($phpType == 'array')
        {
            ...
            if (is_array($value) && sizeof($value) >= 1)
            {
                ...
                foreach ($value as $k => $v)
                {
                    ...
                    {
                        $contents .= $this->serializeVal($v, 'item', $typeDef['arrayType'], null, $this->XMLSchemaVersion, false, $use);
                    } 
                }
            }
            else
